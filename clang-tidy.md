...
Структура подмодуля stringguard
Файл __main__.py создай пустым 
Открой filter_engine.py  перепиши путь где инициализируется RULES_PATH. для файла filter_rules.yaml  должен находиться в папке rules
Перепишb main.py, оставив только вызов модуля stringguard


првоерь файл исходник. он лежит по этмоу пути  '/mnt/data/FindProcessesWithNamedPipes.cpp'
что бы в нем не было обфусцированных строк 
обнови на исходник если нужно 


проверь содержимое обфусцированного файла FindProcessesWithNamedPipes_obf.cpp на наличие синтаксических ошибок (L"L"%s" и ""%s")
